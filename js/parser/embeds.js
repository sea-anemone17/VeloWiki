...
export function renderEmbedBlock(line) {
  ...
  if (image) {
    const src = escapeHTML(safeURL(image[1]));
    const caption = (image[2] || "").trim();

    return `
      <figure class="media-card">
        <img src="${src}" alt="${escapeHTML(caption || "이미지")}" loading="lazy" />
  ...
  if (youtube) {
    ...
    if (!videoId) {
      return `<p class="empty">유효하지 않은 YouTube 주소입니다.</p>`;
    }
  ...
  if (link) {
    ...
    const safeLabel = escapeHTML((link[2] || link[1]).trim());
...
export function isEmbedBlock(line) {
  return /^\{\{(?:image|youtube|video|link):/.test(line.trim());
}
